// Missive JS API variant (vJS5.04)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS5.04 immediately
        this.setBadge('vJS5.04');
    console.log('🔧 Set initial version badge to vJS5.04');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS5.04 once app updates header
        setTimeout(() => this.setBadge('vJS5.04'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS5.04'), 1000);
        setTimeout(() => this.setBadge('vJS5.04'), 2000);
        setTimeout(() => this.setBadge('vJS5.04'), 3000);
        setTimeout(() => this.setBadge('vJS5.04'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS5.04'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS5.04';
          console.log('🧪 Forced version badge to vJS5.04');
        }
