// Missive JS API variant (vJS4.01)
...
class MissiveJSBridge {
  ...
  init() {
    ...
    // Force version badge to vJS4.01 immediately
        this.setBadge('vJS4.01');
    console.log('🔧 Set initial version badge to vJS4.01');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.01 once app updates header
        setTimeout(() => this.setBadge('vJS4.01'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.01'), 1000);
        setTimeout(() => this.setBadge('vJS4.01'), 2000);
        setTimeout(() => this.setBadge('vJS4.01'), 3000);
        setTimeout(() => this.setBadge('vJS4.01'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.01'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.01';
          console.log('🧪 Forced version badge to vJS4.01');
        }
