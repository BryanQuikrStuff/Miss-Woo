// Missive JS API variant (vJS4.14)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.14 immediately
        this.setBadge('vJS4.14');
    console.log('🔧 Set initial version badge to vJS4.14');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.14 once app updates header
        setTimeout(() => this.setBadge('vJS4.14'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.14'), 1000);
        setTimeout(() => this.setBadge('vJS4.14'), 2000);
        setTimeout(() => this.setBadge('vJS4.14'), 3000);
        setTimeout(() => this.setBadge('vJS4.14'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.14'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.14';
          console.log('🧪 Forced version badge to vJS4.14');
        }
