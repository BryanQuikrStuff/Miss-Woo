// Missive JS API variant (vJS4.15)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.15 immediately
        this.setBadge('vJS4.15');
    console.log('🔧 Set initial version badge to vJS4.15');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.15 once app updates header
        setTimeout(() => this.setBadge('vJS4.15'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.15'), 1000);
        setTimeout(() => this.setBadge('vJS4.15'), 2000);
        setTimeout(() => this.setBadge('vJS4.15'), 3000);
        setTimeout(() => this.setBadge('vJS4.15'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.15'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.15';
          console.log('🧪 Forced version badge to vJS4.15');
        }
