// Missive JS API variant (vJS4.11)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.11 immediately
        this.setBadge('vJS4.11');
    console.log('🔧 Set initial version badge to vJS4.11');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.11 once app updates header
        setTimeout(() => this.setBadge('vJS4.11'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.11'), 1000);
        setTimeout(() => this.setBadge('vJS4.11'), 2000);
        setTimeout(() => this.setBadge('vJS4.11'), 3000);
        setTimeout(() => this.setBadge('vJS4.11'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.11'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.11';
          console.log('🧪 Forced version badge to vJS4.11');
        }
