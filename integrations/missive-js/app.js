// Missive JS API variant (vJS4.20)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.20 immediately
        this.setBadge('vJS4.20');
    console.log('🔧 Set initial version badge to vJS4.20');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.20 once app updates header
        setTimeout(() => this.setBadge('vJS4.20'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.20'), 1000);
        setTimeout(() => this.setBadge('vJS4.20'), 2000);
        setTimeout(() => this.setBadge('vJS4.20'), 3000);
        setTimeout(() => this.setBadge('vJS4.20'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.20'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.20';
          console.log('🧪 Forced version badge to vJS4.20');
        }
