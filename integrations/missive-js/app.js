// Missive JS API variant (vJS4.05)
...
class MissiveJSBridge {
  ...
  init() {
    ...
    // Force version badge to vJS4.05 immediately
        this.setBadge('vJS4.05');
    console.log('🔧 Set initial version badge to vJS4.05');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.05 once app updates header
        setTimeout(() => this.setBadge('vJS4.05'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.05'), 1000);
        setTimeout(() => this.setBadge('vJS4.05'), 2000);
        setTimeout(() => this.setBadge('vJS4.05'), 3000);
        setTimeout(() => this.setBadge('vJS4.05'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.05'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.05';
          console.log('🧪 Forced version badge to vJS4.05');
        }
