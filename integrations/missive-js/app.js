// Missive JS API variant (vJS4.19)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.19 immediately
        this.setBadge('vJS4.19');
    console.log('🔧 Set initial version badge to vJS4.19');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.19 once app updates header
        setTimeout(() => this.setBadge('vJS4.19'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.19'), 1000);
        setTimeout(() => this.setBadge('vJS4.19'), 2000);
        setTimeout(() => this.setBadge('vJS4.19'), 3000);
        setTimeout(() => this.setBadge('vJS4.19'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.19'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.19';
          console.log('🧪 Forced version badge to vJS4.19');
        }
