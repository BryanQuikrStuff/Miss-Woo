// Missive JS API variant (vJS4.17)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.17 immediately
        this.setBadge('vJS4.17');
    console.log('🔧 Set initial version badge to vJS4.17');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.17 once app updates header
        setTimeout(() => this.setBadge('vJS4.17'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.17'), 1000);
        setTimeout(() => this.setBadge('vJS4.17'), 2000);
        setTimeout(() => this.setBadge('vJS4.17'), 3000);
        setTimeout(() => this.setBadge('vJS4.17'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.17'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.17';
          console.log('🧪 Forced version badge to vJS4.17');
        }
