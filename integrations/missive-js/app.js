// Missive JS API variant (vJS4.16)
...
class MissiveJSBridge {
  ...
  init() {
    console.log('🚀 Initializing MissiveJSBridge...');
      ...
      // Force version badge to vJS4.16 immediately
        this.setBadge('vJS4.16');
    console.log('🔧 Set initial version badge to vJS4.16');
  ...
  initializeApp() {
    try {
      ...
      if (window.config && window.MissWooApp) {
        ...
        // Override version badge to vJS4.16 once app updates header
        setTimeout(() => this.setBadge('vJS4.16'), 300);
        
        // Additional aggressive version setting to ensure it shows
        setTimeout(() => this.setBadge('vJS4.16'), 1000);
        setTimeout(() => this.setBadge('vJS4.16'), 2000);
        setTimeout(() => this.setBadge('vJS4.16'), 3000);
        setTimeout(() => this.setBadge('vJS4.16'), 5000);
        ...
        if (this.app && this.app.updateHeaderWithVersion) {
          ...
          this.app.updateHeaderWithVersion = () => {
            ...
            setTimeout(() => this.setBadge('vJS4.16'), 100);
          };
        }
  ...
  bindManualSearchEvents() {
    ...
    window.MissWooDebug = {
      ...
      checkVersionBadge: () => {
        ...
        if (el) {
          el.textContent = 'vJS4.16';
          console.log('🧪 Forced version badge to vJS4.16');
        }
