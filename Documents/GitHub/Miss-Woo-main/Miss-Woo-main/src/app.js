...
class MissWooApp {
    ...
    try {
      ...
      this.maxPreloadEmailsOnInboxOpen = 20; // Limit number of emails to preload when inbox opens (most recent)
  ...
  async fetchAndPreloadConversations(conversationIds) {
    ...
    try {
      ...
      // Sort conversations by most recent (newest first) to ensure we preload the most recent emails
      // Conversations from Missive API should have date_updated, updated_at, or similar timestamp fields
      const sortedConversations = [...conversations].sort((a, b) => {
        // Try multiple possible date fields (Missive API may use different field names)
        const dateA = a.date_updated || a.updated_at || a.last_message_date || a.updated || a.date || 0;
        const dateB = b.date_updated || b.updated_at || b.last_message_date || b.updated || b.date || 0;
        
        // Convert to timestamps for comparison
        const timestampA = dateA instanceof Date ? dateA.getTime() : (typeof dateA === 'string' ? new Date(dateA).getTime() : dateA);
        const timestampB = dateB instanceof Date ? dateB.getTime() : (typeof dateB === 'string' ? new Date(dateB).getTime() : dateB);
        
        // Sort descending (newest first)
        return timestampB - timestampA;
      });
      
      // Take only the most recent conversations up to our limit
      const mostRecentConversations = sortedConversations.slice(0, this.maxPreloadEmailsOnInboxOpen);
      console.log(`📅 Sorted ${conversations.length} conversations by date, taking ${mostRecentConversations.length} most recent for preloading`);

      // Extract emails from the most recent conversations
      const emailsToPreload = this.extractEmailsFromConversations(mostRecentConversations);
      console.log(`📧 Extracted ${emailsToPreload.length} unique emails from ${mostRecentConversations.length} most recent conversations`);
      ...
      // OPTIMIZATION: Prioritize first conversation (current one) for preloading
      // Extract first conversation email for prioritization (from original unsorted list to get current conversation)
      let prioritizedEmail = null;
      if (conversations.length > 0) {
        const firstConversation = conversations[0]; // Original first conversation (current one)
        ...
        // Ensure prioritized email is included in the preload list even if it's not in the most recent 20
        if (prioritizedEmail && !emailsToPreload.includes(prioritizedEmail)) {
          emailsToPreload.unshift(prioritizedEmail); // Add to front
          console.log(`⭐ Added prioritized email to preload list: ${prioritizedEmail}`);
        }
      }

      // Limit the number of emails to preload (prioritize current conversation)
      let limitedEmailsToPreload = emailsToPreload;
      if (emailsToPreload.length > this.maxPreloadEmailsOnInboxOpen) {
        // Always include the prioritized email (current conversation) if it exists
        const otherEmails = emailsToPreload.filter(email => email !== prioritizedEmail);
        const limitedOthers = otherEmails.slice(0, this.maxPreloadEmailsOnInboxOpen - (prioritizedEmail ? 1 : 0));
        
        if (prioritizedEmail) {
          limitedEmailsToPreload = [prioritizedEmail, ...limitedOthers];
        } else {
          limitedEmailsToPreload = limitedOthers;
        }
        
        console.log(`📊 Limiting preload to ${limitedEmailsToPreload.length} emails (from ${emailsToPreload.length} total visible)`);
      }
      
      // Preload customer details for limited number of emails
      console.log(`🔄 Starting preload for ${limitedEmailsToPreload.length} emails from visible inbox...`);
      
      // Preload in background but track progress (use normalized prioritized email)
      this.preloadEmailsData(limitedEmailsToPreload, prioritizedEmail)
        .then(() => {
          console.log(`✅ Completed preloading ${limitedEmailsToPreload.length} emails from visible inbox`);
        })
        .catch(error => {
          console.error("❌ Error preloading emails data:", error);
          // OPTIMIZATION: Retry failed preloads after a delay
          setTimeout(() => {
            console.log(`🔄 Retrying preload for ${limitedEmailsToPreload.length} emails after error...`);
            this.preloadEmailsData(limitedEmailsToPreload, prioritizedEmail).catch(retryError => {
              console.error("❌ Retry preload also failed:", retryError);
            });
          }, 5000); // Retry after 5 seconds
        });

      console.log(`✅ Started preloading data for ${limitedEmailsToPreload.length} emails in background`);
    } catch (error) {
